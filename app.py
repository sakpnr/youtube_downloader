...
def progress_hook(d):
    if d['status'] == 'downloading':
        try:
            ...
            if total == 0:  # total_bytes yoksa total_bytes_estimate'i dene
            ...
            if total > 0:
                ...
                # Toplam boyut bilinmiyorsa, indirilen boyutu MB cinsinden göster
...
def setup_ffmpeg():
    ...
    # FFmpeg klasörünü yeniden adlandır
...
def get_download_path(location):
    """Seçilen konuma göre tam dosya yolunu döndürür"""
    home = str(Path.home())
    ...
    return paths.get(location, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'downloads'))
...
def get_video_formats():
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # Kullanılabilir formatları filtrele ve düzenle
            formats = []
            seen_qualities = set()
            
            if download_type == 'audio':
                # Sadece ses formatlarını al
                ...
                for f in audio_formats:
                    abr = f.get('abr', 0)
                    if abr > 0:  # Sadece bit rate'i belli olan formatları al
                        quality = f'{abr}'
                        if quality not in seen_qualities:
                            formats.append({
                                'format_id': f['format_id'],
                                'quality': quality,
                                'ext': 'mp3',  # MP3 olarak kaydedeceğiz
                                'filesize': f.get('filesize', 0),
                                'abr': abr
                            })
                            seen_qualities.add(quality)
                
                # Bit rate'e göre sırala
                formats.sort(key=lambda x: x.get('abr', 0), reverse=True)
                
            else:
                # Video formatlarını al
                for f in info['formats']:
                    # Video içeren formatları kontrol et
                    if f.get('vcodec', 'none') != 'none' and f.get('ext', '') == 'mp4':
                        height = f.get('height', 0)
                        quality = f'{height}p' if height else f.get('format_note', 'unknown')
                        
                        # Eğer ses yoksa ve otomatik ses eklenebiliyorsa
                        ...
                        if quality not in seen_qualities and height >= 360:  # En az 360p ve üzeri
                            formats.append({
                                'format_id': format_id,
                                'quality': quality,
                                'ext': f['ext'],
    ...
    try:
        ...
        if download_type == 'audio':
            ...
            ydl_opts = {
                ...
                'ffmpeg_location': ffmpeg_path,
                'progress_hooks': [progress_hook],
                'merge_output_format': 'mp4'  # Çıktı formatını MP4 olarak zorla
            }
        
        # Video/ses bilgilerini al ve indir
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            # Önce bilgileri al
            info = ydl.extract_info(url, download=False)
            ...
            # İndirilen dosyanın adını bul
            ext = 'mp3' if download_type == 'audio' else 'mp4'  # Video için her zaman mp4
